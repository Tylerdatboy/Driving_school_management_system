<template>
	<div class="addEdit-block" :style='{"padding":"10px 30px"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 30px 0","flexWrap":"wrap","textAlign":"left","background":"none","display":"block","width":"100%","justifyContent":"flex-start"}' class="select" v-if="type!='info'" label="Coach account" prop="jiaolianzhanghao">
					<el-select :disabled="ro.jiaolianzhanghao" @change="jiaolianzhanghaoChange" v-model="ruleForm.jiaolianzhanghao" placeholder="Please select Coach account">
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 30px 0","flexWrap":"wrap","textAlign":"left","background":"none","display":"block","width":"100%","justifyContent":"flex-start"}' class="input" v-else-if="ruleForm.jiaolianzhanghao" label="Coach account" prop="jiaolianzhanghao">
				...
				<el-form-item :style='{"margin":"0 0 30px 0","flexWrap":"wrap","textAlign":"left","background":"none","display":"block","width":"100%","justifyContent":"flex-start"}' class="date" v-if="type!='info'" label="Allocate time" prop="fenpeishijian">
					<el-date-picker
				...
				<el-form-item :style='{"margin":"0 0 30px 0","flexWrap":"wrap","textAlign":"left","background":"none","display":"block","width":"100%","justifyContent":"flex-start"}' class="input" v-else-if="ruleForm.fenpeishijian" label="Allocate time" prop="fenpeishijian">
					<el-input v-model="ruleForm.fenpeishijian" placeholder="Allocate time" readonly></el-input>
				</el-form-item>
			</template>
	...
	methods: {
		...
		init(id,type) {
			...
			jiaolianzhanghaoChange () {
				this.$http({
					url: `follow/jiaxiaojiaolian/jiaolianzhanghao?columnValue=`+ this.ruleForm.jiaolianzhanghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					 ...
					 this.$http({
					   url: `fenpeijiaolian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fenpeijiaolianCrossAddOrUpdateFlag = false;
						 this.parent.search();
