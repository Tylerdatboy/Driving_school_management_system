...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 7% 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 10px","display":"flex"}'>
        ...
        <el-select v-model="formSearch.jiaolianzhanghao" placeholder="Please select Coach account" :clearable="true">
          <el-option v-for="(item, index) in jiaolianzhanghaoOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 10px","display":"flex"}'>
	  ...
	  <el-button v-if="isAuth('fenpeijiaolian','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 35px","margin":"0 10px 0 0","color":"#fff","minWidth":"90px","outline":"none","borderRadius":"0","background":"#20ad9e","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/fenpeijiaolianAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/jiaxiaojiaolian/jiaolianzhanghao').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get('fenpeijiaolian/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/fenpeijiaolianDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
