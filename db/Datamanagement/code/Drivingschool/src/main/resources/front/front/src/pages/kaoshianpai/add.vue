...
<div :style='{"width":"100%","padding":"40px 7%","margin":"0 auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"linear-gradient(90deg, rgba(238,177,238,1) 0%, rgba(165,228,222,1) 30%, rgba(165,228,222,1) 100%)","display":"flex"}' label="Exam time" prop="kaoshishijian" >
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"linear-gradient(90deg, rgba(238,177,238,1) 0%, rgba(165,228,222,1) 30%, rgba(165,228,222,1) 100%)","display":"flex"}'  label="Exam Subjects" prop="kaoshikemu">
            <el-select v-model="ruleForm.kaoshikemu" placeholder="Please select Exam Subjects"  >
              <el-option
                  v-for="(item,index) in kaoshikemuOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"linear-gradient(90deg, rgba(238,177,238,1) 0%, rgba(165,228,222,1) 30%, rgba(165,228,222,1) 100%)","display":"flex"}' label="Exam place" prop="kaoshididian">
            <el-input v-model="ruleForm.kaoshididian"
                placeholder="Exam place" clearable ></el-input>
          </el-form-item>
...
<script>
  export default {
    data() {
      return {
        ...
        rules: {
          ...
          kaoshishijian: [
            { required: true, message: 'Exam timeCannot be empty', trigger: 'blur' },
          ],
          kaoshikemu: [
          ],
          kaoshididian: [
            { required: true, message: 'Exam placeCannot be empty', trigger: 'blur' },
          ],
        },
      };
    },
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('kaoshianpai/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('kaoshianpai/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('kaoshianpai/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
